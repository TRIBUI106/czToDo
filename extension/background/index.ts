...
async function handleMessage(message: Message): Promise<any> {
  ...
  switch (message.type) {
    ...
    case 'CAPTURE_EMAIL': {
      const { subject, from, body, url } = message.payload;
      ...
      const newTodo: Partial<Todo> = {
        title: subject || 'Email from ' + from || 'Captured email',
        description: body
    ...
    case 'CAPTURE_SLACK_MESSAGE': {
      const { messageText, channel, author, url } = message.payload;
      ...
      const newTodo: Partial<Todo> = {
        title: messageText?.substring(0, 100) || `Slack message in ${channel}`,
        description: messageText
          ? `From: ${author} in #${channel}\n\n${messageText.substring(0, 500)}`
          : `From: ${author} in #${channel}`,
        source_url: url,
        source_type: 'slack',
        status: 'active',
